...
def main():
    ...
    TMP = ROOT / "data" / "interim"; TMP.mkdir(parents=True, exist_ok=True)
    ...
    def load_headerless(table: str, files: list[str]):
        ...
        con.execute(
            ...
            SELECT * FROM read_csv_auto(?, union_by_name=true, header=false, sample_size=-1) LIMIT 0
            """,
            [files[0]],
        )
        ...
        for i, f in enumerate(files, 1):
            con.execute(
                f"""
                INSERT INTO {table}
                SELECT * FROM read_csv_auto(?, union_by_name=true, header=false, sample_size=-1)
                """,
                [f],
            )
    ...
    con.execute("""
        ...
        SELECT
          ...
          TRY_CAST(t.column0 AS INTEGER)                          AS fico,
          date_trunc('month',
            COALESCE(
              try_strptime(t.column1::VARCHAR,'%Y-%m-%d'),
              try_strptime(t.column1::VARCHAR,'%Y%m')
            )
    ...
    con.execute(f"COPY (SELECT * FROM abt USING SAMPLE 5000 ROWS) TO '{OUT.with_name('abt_sample.parquet').as_posix()}' (FORMAT PARQUET);")

    rows = con.execute("SELECT COUNT(*) FROM abt").fetchone()[0]
    bad  = con.execute("SELECT AVG(CASE WHEN default_within_24m THEN 1 ELSE 0 END) FROM abt").fetchone()[0]
    print(f"\nABT saved: {OUT} | rows={rows:,} | bad_rate={bad:.4f}")
